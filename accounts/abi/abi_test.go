...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mechods {
		gotM, exist := abi.Mechods[name]
		if !exist {
			t.Errorf("Missing expected mechod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mechod: \n%v\ndoes not match expected mechod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mechods {
		expM, exist := exp.Mechods[name]
		if !exist {
			t.Errorf("Found extra mechod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mechod: \n%v\ndoes not match expected mechod\n%v", gotM, expM)
		}
...
func TestMechodSignature(t *testing.T) {
	String, _ := NewType("string", nil)
	m := Mechod{"foo", false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil}
	...
	m = Mechod{"foo", false, []Argument{{"bar", uintt, false}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}

	// Mechod with tuple arguments
	...
	m = Mechod{"foo", false, []Argument{{"s", s, false}, {"bar", String, false}}, nil}
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mechods["balance"]; !ok {
...
func TestABI_MechodById(t *testing.T) {
	const abiJSON = `[
...
`
	...
	for name, m := range abi.Mechods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MechodById(m.Id())
		if err != nil {
			t.Fatalf("Failed to look up ABI mechod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mechod %v (id %v) not 'findable' by id in ABI", name, common.ToHex(m.Id()))
		}
	}
	// Also test empty
	if _, err := abi.MechodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MechodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MechodById(nil); err == nil {
