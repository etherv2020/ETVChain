...
// See https://github.com/etvchaineum/wiki/wiki/Web3-Secret-Storage-Definition for more information.
package keystore
...
type KeyStore struct {
	...
	updating    bool                    // Whetvchain the event notification loop is running
...
// HasAddress reports whetvchain a key with the given address is present.
...
// NewAccountEx generates a new key and stores it into the key directory,
// encrypting it with the passphrase.
func (ks *KeyStore) NewAccountEx(passphrase string) (*ecdsa.PrivateKey,accounts.Account, error) {
	...
	ks.cache.add(account)
	ks.refreshWallets()
	return key.PrivateKey,account, nil
}
...
// ImportPreSaleKey decrypts the given Etvchain presale wallet and stores
