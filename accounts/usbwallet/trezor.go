...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whetvchain the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whetvchain the Trezor is opened, closed
// or whetvchain the Etvchain app was not started on it.
...
// Etvchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EtvchainAddress)
	if _, err := w.trezorExchange(&trezor.EtvchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.EtvchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EtvchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mechod will also return the index of the destination object used.
