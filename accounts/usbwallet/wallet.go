...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Etvchain
...
type wallet struct {
	...
	deriveChain    etvchaineum.ChainStateReader // Blockchain state reader to discover used account with
...
// whetvchain they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Etvchain account
			...
			// If the next account is empty, stop self-derivation, but add it nonecheless
...
// Contains implements accounts.Wallet, returning whetvchain a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain etvchaineum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mechod will always return an error.
...
// data is not supported for Ledger wallets, so this mechod will always return
// an error.
