...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 etvchain` in value to maximum `1 etvchain` per 24h period
...
A ruleset file is implemented as a `js` file. Under the hood, the ruleset-engine is a `SignerUI`, implementing the same mechods as the `json-rpc` mechods
...
invokes the corresponding mechod. In doing so, there are three possible outcomes:

1. JS returns "Approve"
  * Auto-approve request
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or someching else is returned
...
implemented for `gech`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whetvchain any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
