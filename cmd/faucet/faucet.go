...
// faucet is a Etvchain faucet backed by a light client.
package main
...
var (
	...
	echPortFlag = flag.Int("echport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Etvchain protocol")
	statsFlag   = flag.String("echstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Etvchains to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Etvchain and the faucet")
)

var (
	etvchain = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Etvchains", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *echPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Etvchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Etvchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Etvchain protocol stack
	client *echclient.Client   // Client connection to the Etvchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Etvchain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := ech.DefaultConfig
	...
	// Assemble the echstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEtvchain
			ctx.Service(&serv)
			return echstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := echclient.NewClient(api)
...
// close terminates the Etvchain connection and tears down the faucet.
...
// apiHandler handles requests for Etvchain grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, etvchain),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Etvchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Someching funky happened, please open an issue at https://github.com/etvchaineum/go-etvchaineum/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), etvchain)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, etvchain)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and Etvchain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Etvchain address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Etvchain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Etvchain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Etvchain address,
