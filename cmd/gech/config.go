...
type echstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gechConfig struct {
	Eth       ech.Config
	Shh       whisper.Config
	Node      node.Config
	Ethstats  echstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *gechConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "ech", "shh")
	cfg.WSModules = append(cfg.WSModules, "ech", "shh")
	cfg.IPCPath = "gech.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gechConfig) {
	// Load defaults.
	cfg := gechConfig{
		Eth:       ech.DefaultConfig,
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the Etvchain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
