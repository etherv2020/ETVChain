...
import (
	...
	math2 "github.com/etvchaineum/go-etvchaineum/common/math"
...
// alechGenesisSpec represents the genesis specification format used by the
// C++ Etvchain implementation.
type alechGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alechGenesisSpecAccount `json:"accounts"`
}

// alechGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alechGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alechGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alechGenesisSpecBuiltin is the precompiled contract definition.
type alechGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *alechGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alechGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlechGenesisSpec converts a go-etvchaineum genesis block into a Alech-specific
// chain specification format.
func newAlechGenesisSpec(network string, genesis *core.Genesis) (*alechGenesisSpec, error) {
	// Only echash is currently supported between go-etvchaineum and alech
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alech format
	spec := &alechGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(echash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alechGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alechGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alechGenesisSpecBuiltin{Name: "sha256",
		Linear: &alechGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alechGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alechGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alechGenesisSpecBuiltin{Name: "identity",
		Linear: &alechGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alechGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &alechGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alechGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alechGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alechGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alechGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *alechGenesisSpec) setPrecompile(address byte, data *alechGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alechGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alechGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alechGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alechGenesisSpecAccount)
	}
...
func (spec *alechGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *alechGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-etvchaineum genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only echash is currently supported between go-etvchaineum and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// gech has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Etvchain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Etvchain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Etvchain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(echash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(echash.ConstantinopleBlockReward)
...
// pyEtvchainGenesisSpec represents the genesis specification format used by the
// Python Etvchain implementation.
type pyEtvchainGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyEtvchainGenesisSpec converts a go-etvchaineum genesis block into a Parity specific
// chain specification format.
func newPyEtvchainGenesisSpec(network string, genesis *core.Genesis) (*pyEtvchainGenesisSpec, error) {
	// Only echash is currently supported between go-etvchaineum and pyetvchaineum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyEtvchainGenesisSpec{
