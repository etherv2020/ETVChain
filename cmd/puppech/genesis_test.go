...
// Tests the go-etvchaineum to Alech chainspec conversion for the Stureby testnet.
func TestAlechSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gech.json")
	if err != nil {
	...
	spec, err := newAlechGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alech.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alechGenesisSpec{}
...
// Tests the go-etvchaineum to Parity chainspec conversion for the Stureby testnet.
