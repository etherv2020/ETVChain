...
// echstatsDockerfile is the Dockerfile required to build an echstats backend
// and associated monitoring site.
var echstatsDockerfile = `
FROM puppech/echstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// echstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an echstats monitoring site.
var echstatsComposefile = `
version: '2'
services:
  echstats:
    build: .
    image: {{.Network}}/echstats
    container_name: {{.Network}}_echstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new echstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(echstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(echstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the echstats service
	if nocache {
...
// echstatsInfos is returned from an echstats status check to allow reporting
// various configuration parameters.
type echstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *echstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an echstats server to verify whetvchain
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*echstatsInfos, error) {
	// Inspect a possible echstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_echstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
