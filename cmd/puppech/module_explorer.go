...
FROM puppech/explorer:latest

ADD echstats.json /echstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ech-net-intelligence-api && pm2 start /echstats.json)' >  explorer.sh && \
	echo '(cd ../etvchainchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the echstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.etvchaineum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	echstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(echstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.echstats[:strings.Index(config.echstats, ":")],
		"Secret": config.echstats[strings.Index(config.echstats, ":")+1 : strings.Index(config.echstats, "@")],
		"Host":   config.echstats[strings.Index(config.echstats, "@")+1:],
	})
	files[filepath.Join(workdir, "echstats.json")] = echstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.etvchaineum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		echstats: infos.envvars["STATS"],
	}
	return stats, nil
}
