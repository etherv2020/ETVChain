...
FROM puppech/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gech --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gech --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --echstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppechNetworkID/{{.NetworkID}}/g' dist/js/etvchainwallet-master.js && \
	sed -i 's/PuppechNetwork/{{.Network}}/g'     dist/js/etvchainwallet-master.js && \
	sed -i 's/PuppechDenom/{{.Denom}}/g'         dist/js/etvchainwallet-master.js && \
	sed -i 's/PuppechHost/{{.Host}}/g'           dist/js/etvchainwallet-master.js && \
	sed -i 's/PuppechRPCPort/{{.RPCPort}}/g'     dist/js/etvchainwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whetvchain
// it's running, and if yes, whetvchain it's responsive.
