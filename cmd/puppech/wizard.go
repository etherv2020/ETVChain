...
// config contains all the configurations needed by puppech that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	echstats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppech configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Etvchain services known to be running on servers
...
// converts it to an Etvchain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by echstats, which compares IPs textually, not by value.
