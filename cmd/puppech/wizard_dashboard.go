...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"echstats", "explorer", "wallet", "faucet"} {
	...
	// If we have echstats running, ask whetvchain to make the secret public or not
	if w.conf.echstats != "" {
		fmt.Println()
		fmt.Println("Include echstats secret on dashboard (y/n)? (default = yes)")
