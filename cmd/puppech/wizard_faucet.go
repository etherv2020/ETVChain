...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Etvchains to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.echstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.echstats = w.readString() + ":" + w.conf.echstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.echstats)
		infos.node.echstats = w.readDefaultString(infos.node.echstats) + ":" + w.conf.echstats
	}
