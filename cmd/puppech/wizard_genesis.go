...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of echash, we're pretty much done
...
// importGenesis imports a Gech genesis spec into puppech.
...
func (w *wizard) manageGenesis() {
	// Figure out whetvchain to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alech.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppech and Gech
		gechJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gechJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gechJson)

		// Export the genesis spec used by Alech (formerly C++ Etvchain)
		if spec, err := newAlechGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alech chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alech", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly EtvchainJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not someching I can do")
		return
	}
}
