...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for echstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["echstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["echstats"] = infos.Report()
		echstats = infos.config
	}
