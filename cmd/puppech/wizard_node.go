...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.echstats == "" {
		log.Error("No echstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.echashdir == "" {
			fmt.Printf("Where should the echash mining DAGs be stored on the remote machine?\n")
			infos.echashdir = w.readString()
		} else {
			fmt.Printf("Where should the echash mining DAGs be stored on the remote machine? (default = %s)\n", infos.echashdir)
			infos.echashdir = w.readDefaultString(infos.echashdir)
		}
	}
	...
	if infos.echstats == "" {
		...
		infos.echstats = w.readString() + ":" + w.conf.echstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.echstats)
		infos.echstats = w.readDefaultString(infos.echstats) + ":" + w.conf.echstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an etvchainbase to mine against
			fmt.Println()
			if infos.etvchainbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.etvchainbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.etvchainbase)
				infos.etvchainbase = w.readDefaultAddress(common.HexToAddress(infos.etvchainbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Etvchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
