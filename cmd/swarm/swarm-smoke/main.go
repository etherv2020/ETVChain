...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.BoolFlag{
			Name:        "single",
			Usage:       "whetvchain to fetch content from a single node or from all nodes",
			Destination: &single,
		},
		cli.IntFlag{
			Name:        "track-timeout",
...
func emitMetrics(ctx *cli.Context) error {
	if gechmetrics.Enabled {
		...
		return influxdb.InfluxDBWithTagsOnce(gechmetrics.DefaultRegistry, endpoint, database, username, password, "swarm-smoke.", tagsMap)
	}
