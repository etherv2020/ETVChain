...
// mapped from textual names used in the tests below to actual Etvchain private
...
// address retrieves the Etvchain address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Etvchain address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/etvchaineum/go-etvchaineum/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gech the header and prepare it for signing
			header := block.Header()
			if j > 0 {
