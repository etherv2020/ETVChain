...
// Tests that echash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	echash := NewTester(nil, false)
	defer echash.Close()

	results := make(chan *types.Block)
	err := echash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := echash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/etvchaineum/go-etvchaineum/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "echash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := echash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	echash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := echash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	if _, err := api.GetWork(); err != errEthashStopped {
		t.Error("expect to return an error to indicate echash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped echash")
	}
}
