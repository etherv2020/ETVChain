...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid echash solution.
	staleThreshold = 7
)
...
func (echash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if echash.config.PowMode == ModeFake || echash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", echash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if echash.shared != nil {
		return echash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	echash.lock.Lock()
	threads := echash.threads
	if echash.rand == nil {
		...
		echash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	echash.lock.Unlock()
	if threads == 0 {
	...
	if echash.workCh != nil {
		echash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			echash.mine(block, id, nonce, abort, locals)
		}(i, uint64(echash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", echash.SealHash(block.Header()))
			}
			close(abort)
		case <-echash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := echash.Seal(chain, block, results, stop); err != nil {
...
func (echash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = echash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = echash.dataset(number, false)
	)
	...
	logger.Trace("Started echash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
...
func (echash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := echash.SealHash(block.Header())
	...
	// whetvchain the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := echash.verifySeal(nil, header, true); err != nil {
	...
	for {
		...
		case result := <-echash.submitWorkCh:
		...
		case errc := <-echash.exitCh:
			// Exit remote loop if echash is closed and return relevant error.
			errc <- nil
