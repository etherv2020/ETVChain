...
// Tests whetvchain remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom echash engine
	echash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer echash.Close()
	...
	echash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := echash.SealHash(header).Hex(); work[0] != want {
...
// Tests whetvchain stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	echash := NewTester(nil, true)
	defer echash.Close()
	api := &API{echash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			echash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, echash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch echash result timeout", id+1)
		}
	}
}
