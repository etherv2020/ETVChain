...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/accounts/usbwallet"
	"github.com/etvchaineum/go-etvchaineum/log"
	"github.com/etvchaineum/go-etvchaineum/rpc"
	"github.com/robertkrimen/otto"
)

// bridge is a collection of JavaScript utility mechods to bride the .js runtime
// environment and the Go RPC connection backing the remote mechod calls.
type bridge struct {
...
// NewAccount is a wrapper around the personal.newAccount RPC mechod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mechod (saved in jech.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jech.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jech.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jech.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}

// UnlockAccount is a wrapper around the personal.unlockAccount RPC mechod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mechod (saved in jech.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jech.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// Sign is a wrapper around the personal.sign RPC mechod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mechod (saved in
// jech.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jech.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("ech.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the web3 provider "send" mechod.
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	...
	for _, req := range reqs {
		...
		err = b.client.Call(&result, req.Mechod, req.Params...)
		switch err := err.(type) {
		case nil:
			if result == nil {
