...
func newTester(t *testing.T, confOverride func(*ech.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Etvchain service within
	...
	echConf := &ech.Config{
		...
		Etvchainbase: common.HexToAddress(testAddress),
		Ethash: echash.Config{
			PowMode: echash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(echConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ech.New(ctx, echConf) }); err != nil {
		t.Fatalf("failed to register Etvchain protocol: %v", err)
	}
	...
	stack.Service(&etvchaineum)

	return &tester{
		workspace: workspace,
		stack:     stack,
		etvchaineum:  etvchaineum,
		console:   console,
		input:     prompter,
		output:    printer,
	}
