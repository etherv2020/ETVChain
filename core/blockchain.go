...
// Package core implements the Etvchain consensus protocol.
package core
...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/common/prque"
	...
	"github.com/etvchaineum/go-etvchaineum/trie"
...
type CacheConfig struct {
	Disabled       bool          // Whetvchain to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     echdb.Database // Low level persistent database to store final content in
	...
	shouldPreserve func(*types.Block) bool // Function used to determine whetvchain should preserve the given block.
}

// NewBlockChain returns a fully initialised block chain using information
// available in the database. It initialises the default Etvchain Validator and
// Processor.
func NewBlockChain(db echdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
...
// loadLastState loads the last known chain state from the database. This mechod
...
//
// This mechod only rolls back the current block. The current header and current
...
// insert injects a new head block into the current block chain. This mechod
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= echdb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - echdb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have someching meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
	// Do a sanity check that the provided chain is actually ordered and linked
...
//
// This mechod is split out so that import batches that require re-injecting
...
// found.
//
// The mechod writes all (header-and-body-valid) blocks to disk, then tries to
...
//
// The verify parameter can be used to fine tune whetvchain nonce verification
...
//
// Note: This mechod is not concurrent-safe with inserting blocks simultaneously
