...
// Trie is a Etvchain Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb echdb.Putter) error
}
...
func NewDatabase(db echdb.Database) Database {
...
func NewDatabaseWithCache(db echdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb echdb.Putter) error {
