...
// Interpreter is used to run Etvchain based contracts and will utilise the
...
// keccakState wraps sha3.state. In addition to the usual hash mechods, it also supports
...
type EVMInterpreter struct {
	...
	readOnly   bool   // Whetvchain to throw on stateful modifications
	returnData []byte // Last CALL's return data for subsequent reuse
}

// NewEVMInterpreter returns a new instance of the Interpreter.
func NewEVMInterpreter(evm *EVM, cfg Config) *EVMInterpreter {
	// We use the STOP instruction whetvchain to see
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		// cost is explicitly set so that the capture state defer mechod can get the proper cost
