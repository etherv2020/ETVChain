...
// Package etvchaineum defines interfaces for interacting with Etvchain.
package etvchaineum
...
// NotFound is returned by API mechods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mechods in this interface access raw
...
type ChainReader interface {
	...
	// This mechod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type TransactionReader interface {
	...
	// blockchain. The isPending return value indicates whetvchain the transaction has been
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mechod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mechod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by ech_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mechod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mechod can be used to retrieve the
