...
type TestCmd struct {
	// For total convenience, all testing mechods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gech-test" in cmd/gech/run_test.go)
...
// This mechod can also be called from an expect template, e.g.:
//
//     gech.expect(`Passphrase: {{.InputLine "password"}}`)
