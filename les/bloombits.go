...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Etvchain
...
func (ech *LightEtvchain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-ech.shutdownChan:
					return

				case request := <-ech.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					compVectors, err := light.GetBloomBits(task.Context, ech.odr, task.Bit, task.Sections)
					if err == nil {
