...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/les/flowcontrol"
...
func testIndexers(db echdb.Database, odr light.OdrBackend, iConfig *light.IndexerConfig) (*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer) {
	chtIndexer := light.NewChtIndexer(db, odr, iConfig.ChtSize, iConfig.ChtConfirms)
	bloomIndexer := ech.NewBloomIndexer(db, iConfig.BloomSize, iConfig.BloomConfirms)
...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), odr *LesOdr, peers *peerSet, db echdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		gchain, _ := core.GenerateChain(gspec.Config, genesis, echash.NewFaker(), db, blocks, generator)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), odr *LesOdr, peers *peerSet, db echdb.Database) *ProtocolManager {
...
func newClientServerEnv(t *testing.T, blocks int, protocol int, waitIndexers func(*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer), newPeer bool) (*TestEntity, *TestEntity, func()) {
	db, ldb := echdb.NewMemDatabase(), echdb.NewMemDatabase()
