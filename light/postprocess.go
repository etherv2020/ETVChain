...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/common/bitutil"
...
func GetChtRoot(db echdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db echdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    echdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db echdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := echdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, echdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db echdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db echdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db echdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := echdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, echdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
