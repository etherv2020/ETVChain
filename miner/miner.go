...
// Package miner implements Etvchain block creation and mining.
package miner
...
// Backend wraps all mechods required for mining.
...
type Miner struct {
	...
	canStart    int32 // can start indicates whetvchain we can start the mining operation
	shouldStart int32 // should start indicates whetvchain we should start after sync
}

func New(ech Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		ech:      ech,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, engine, ech, mux, recommit, gasFloor, gasCeil, isLocalBlock),
		canStart: 1,
	}
	go miner.update()
...
// change between multiple mechod calls
func (self *Miner) PendingBlock() *types.Block {
	return self.worker.pendingBlock()
}

func (self *Miner) SetEtvchainbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setEtvchainbase(addr)
}
