...
func main() {
	...
	// Pre-generate the echash mining DAG so we don't race
	echash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".echash"))
	...
	for _, node := range nodes {
		...
		if err := node.Service(&etvchaineum); err != nil {
			panic(err)
		}
		if err := etvchaineum.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var etvchaineum *ech.Etvchain
		if err := nodes[index%len(nodes)].Service(&etvchaineum); err != nil {
			panic(err)
		}
		...
		if err := etvchaineum.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := etvchaineum.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Etvchain node
	...
	// Start the node and configure a full Etvchain node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return ech.New(ctx, &ech.Config{
			Genesis:         genesis,
			...
			GPO:             ech.DefaultConfig.GPO,
			Ethash:          ech.DefaultConfig.Ethash,
