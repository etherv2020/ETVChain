...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gech.LightScryptN, Gech.LightScryptP);
		...
		try {
			...
			// data from this mechod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.etvchaineum", new NodeConfig());
			node.start();
			...
			EtvchainClient ec = node.getEtvchainClient();
	...
	// signatures too. Regression test for go-etvchaineum issue #14599.
...
//
// This mechod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogetvchain
	...
	workspace, err := ioutil.TempDir("", "gech-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/etvchaineum/gechtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gech and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.etvchaineum", "github.com/etvchaineum/go-etvchaineum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gech.aar"), "gech.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "etvchaineum", "gechtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.etvchaineum.gechtest"
