...
// Package gech contains the simplified mobile APIs to go-etvchaineum.
//
// The scope of this package is *not* to allow writing a custom Etvchain client
// with pieces plucked from go-etvchaineum, rather to allow writing native dapps on
...
// from the go-etvchaineum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-etvchaineum version.
// This is needed because gomobile cannot expose named types for now.
//
// Whenever a mechod argument or a return type is a custom struct, the pointer
...
// a go slice with the mechods `Size`, `Get` and `Set`. Further slice operations
...
//
// If a mechod has multiple return values (e.g. some return + an error), those
