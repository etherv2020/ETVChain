...
// Tests whetvchain services can be registered and duplicates caught.
...
func TestAPIGather(t *testing.T) {
	...
	makeAPI := func(result string) *OneMechodAPI {
		return &OneMechodAPI{fun: func() { calls <- result }}
	}
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	...
	tests := []struct {
		...
		{"single_theOneMechod", "single.v1"},
		{"multi_theOneMechod", "multi.v1"},
		{"multi.v2_theOneMechod", "multi.v2"},
		{"multi.v2.nested_theOneMechod", "multi.v2.nested"},
	}
	for i, test := range tests {
		if err := client.Call(nil, test.Mechod); err != nil {
