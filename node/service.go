...
// as well as utility mechods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (echdb.Database, error) {
	if ctx.config.DataDir == "" {
		return echdb.NewMemDatabase(), nil
	}
	db, err := echdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
