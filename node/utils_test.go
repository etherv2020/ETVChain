...
// Set of services all wrapping the base NoopService resulting in the same mechod
...
// mechods can be instrumented both return value as well as event hook wise.
...
// InstrumentingWrapper is a mechod to specialize a service constructor returning
...
// same mechod signatures but different outer types.
...
// OneMechodAPI is a single-mechod API handler to be returned by test services.
type OneMechodAPI struct {
	fun func()
}

func (api *OneMechodAPI) TheOneMechod() {
	if api.fun != nil {
		api.fun()
	}
}
