...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togetvchain.
//
...
// Connect connects two nodes togetvchain by calling the "admin_addPeer" RPC
// mechod on the "one" node so that it connects to the "other" node
...
// mechod on the "one" node so that it disconnects from the "other" node
...
// NOTE:
// it also checks whetvchain there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whetvchain or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/etvchainsphere/go-etvchaineum/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whetvchain or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whetvchain both nodes are currently up
