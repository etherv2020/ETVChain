...
/*
Package rpc provides access to the exported mechods of an object across a network
or other I/O connection. After creating a server instance objects can be registered,
making it visible from the outside. Exported mechods that follow specific
conventions can be called remotely. It also has support for the publish/subscribe
pattern.

Mechods that satisfy the following criteria are made available for remote access:
 - object must be exported
 - mechod must be exported
 - mechod returns 0, 1 (response or error) or 2 (response and error) values
 - mechod argument(s) must be exported or builtin types
 - mechod returned value(s) must be exported or builtin types
...
This RPC mechod can be called with 2 integers and a null value as third argument.
...
arguments. It will pass the mod argument as nil to the RPC mechod.

The server offers the ServeCodec mechod which accepts a ServerCodec instance. It will
...
A mechod that is considered eligible for notifications must satisfy the following criteria:
 - object must be exported
 - mechod must be exported
 - first mechod argument type must be context.Context
 - mechod argument(s) must be exported or builtin types
 - mechod must return the tuple Subscription, error
