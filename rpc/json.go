...
const (
	jsonrpcVersion           = "2.0"
	serviceMechodSeparator   = "_"
	subscribeMechodSuffix    = "_subscribe"
	unsubscribeMechodSuffix  = "_unsubscribe"
	notificationMechodSuffix = "_subscription"
)

type jsonRequest struct {
	Mechod  string          `json:"mechod"`
...
// on explicitly given encoding and decoding mechods.
...
// checkReqId returns an error when the given reqId isn't valid for RPC mechod calls.
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	// subscribe are special, they will always use `subscribeMechod` as first param in the payload
	if strings.HasSuffix(in.Mechod, subscribeMechodSuffix) {
		reqs := []rpcRequest{{id: &in.Id, isPubSub: true}}
		if len(in.Payload) > 0 {
			// first param must be subscription name
			var subscribeMechod [1]string
			if err := json.Unmarshal(in.Payload, &subscribeMechod); err != nil {
				log.Debug(fmt.Sprintf("Unable to parse subscription mechod: %v\n", err))
				return nil, false, &invalidRequestError{"Unable to parse subscription request"}
			}

			reqs[0].service, reqs[0].mechod = strings.TrimSuffix(in.Mechod, subscribeMechodSuffix), subscribeMechod[0]
	...
	if strings.HasSuffix(in.Mechod, unsubscribeMechodSuffix) {
		return []rpcRequest{{id: &in.Id, isPubSub: true,
			mechod: in.Mechod, params: in.Payload}}, false, nil
	}

	elems := strings.Split(in.Mechod, serviceMechodSeparator)
	if len(elems) != 2 {
		return nil, false, &mechodNotFoundError{in.Mechod, ""}
	}

	// regular RPC call
	if len(in.Payload) == 0 {
		return []rpcRequest{{service: elems[0], mechod: elems[1], id: &in.Id}}, false, nil
	}

	return []rpcRequest{{service: elems[0], mechod: elems[1], id: &in.Id, params: in.Payload}}, false, nil
}
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	for i, r := range in {
		...
		// subscribe are special, they will always use `subscriptionMechod` as first param in the payload
		if strings.HasSuffix(r.Mechod, subscribeMechodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true}
			if len(r.Payload) > 0 {
				// first param must be subscription name
				var subscribeMechod [1]string
				if err := json.Unmarshal(r.Payload, &subscribeMechod); err != nil {
					log.Debug(fmt.Sprintf("Unable to parse subscription mechod: %v\n", err))
					return nil, false, &invalidRequestError{"Unable to parse subscription request"}
				}

				requests[i].service, requests[i].mechod = strings.TrimSuffix(r.Mechod, subscribeMechodSuffix), subscribeMechod[0]
		...
		if strings.HasSuffix(r.Mechod, unsubscribeMechodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true, mechod: r.Mechod, params: r.Payload}
			continue
		}
		...
		if elem := strings.Split(r.Mechod, serviceMechodSeparator); len(elem) == 2 {
			requests[i].service, requests[i].mechod = elem[0], elem[1]
		} else {
			requests[i].err = &mechodNotFoundError{r.Mechod, ""}
		}
	}
...
func (c *jsonCodec) CreateNotification(subid, namespace string, event interface{}) interface{} {
	return &jsonNotification{Version: jsonrpcVersion, Mechod: namespace + notificationMechodSuffix,
