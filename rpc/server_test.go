...
func testServerMechodExecution(t *testing.T, mechod string) {
	server := NewServer()
	service := new(Service)
	...
	go server.ServeCodec(NewJSONCodec(serverConn), OptionMechodInvocation)
...
func TestServerMechodExecution(t *testing.T) {
	testServerMechodExecution(t, "echo")
}

func TestServerMechodWithCtx(t *testing.T) {
	testServerMechodExecution(t, "echoWithCtx")
}
