...
func verify(t *testing.T, jsondata, calldata string, exp []interface{}) {
	...
	data, err := mechod.Inputs.UnpackValues(argdata)
	if err != nil {
		t.Fatal(err)
	}
	if len(data) != len(exp) {
...
func TestNewUnpacker(t *testing.T) {
	...
	testcases := []unpackTest{
		...
		}, { // https://github.com/etvchaineum/wiki/wiki/Etvchain-Contract-ABI#examples
...
[
	...
	for i, hexdata := range []string{
		...
		// From https://github.com/etvchaineum/wiki/wiki/Etvchain-Contract-ABI
...
func TestSelectorUnmarshalling(t *testing.T) {
	...
	for id, selector := range db.db {
		...
		abistring, err = MechodSelectorToAbi(selector)
		if err != nil {
			t.Error(err)
			return
		}
		...
		m, err := abistruct.MechodById(common.Hex2Bytes(id[2:]))
		if err != nil {
			t.Error(err)
			return
		}
...
func TestCustomABI(t *testing.T) {
	...
	_, err = abidb.LookupMechodSelector(calldata)
	if err == nil {
	...
	_, err = abidb2.LookupMechodSelector(calldata)
	if err != nil {
