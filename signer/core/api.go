...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mechodSelector *string) (*echapi.SignTransactionResult, error)
...
// SignerUI specifies what mechod a UI needs to implement to be able to be used as a UI for the signer
type SignerUI interface {
	...
	// This mechod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx echapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mechodSelector *string) (*echapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(&args, mechodSelector)
	if err != nil {
		return nil, err
	}
	...
	response := echapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// OBS! This mechod is removed from the public API. It should not be exposed on the external API
