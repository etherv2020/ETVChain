...
func (ui *HeadlessUI) OnApprovedTx(tx echapi.SignTransactionResult) {
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "ech-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *echapi.SignTransactionResult
		err       error
	)
	...
	control <- "Y"
	control <- "wrongpassword"
	res, err = api.SignTransaction(context.Background(), tx, &mechodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mechodSig)
	if err != nil {
		t.Fatal(err)
	}
