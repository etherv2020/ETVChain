...
func testSelector(selector string, data []byte) (*decodedCallData, error) {
	...
	abiData, err := MechodSelectorToAbi(selector)
	if err != nil {
		return nil, err
	}
...
// validateCallData checks if the ABI-data + mechodselector (if given) can be parsed and seems to match
func (v *Validator) validateCallData(msgs *ValidationMessages, data []byte, mechodSelector *string) {
	if len(data) == 0 {
		return
	}
	if len(data) < 4 {
	...
	if mechodSelector != nil {
		info, err = testSelector(*mechodSelector, data)
		if err != nil {
			...
			v.db.AddSignature(*mechodSelector, data[:4])
		}
		return
	}
	// Check the db
	selector, err := v.db.LookupMechodSelector(data[:4])
	if err != nil {
...
func (v *Validator) validate(msgs *ValidationMessages, txargs *SendTxArgs, mechodSelector *string) error {
	...
	if txargs.To == nil {
		...
		// e.g. https://github.com/etvchaineum/go-etvchaineum/issues/16106
		if len(data) == 0 {
			if txargs.Value.ToInt().Cmp(big.NewInt(0)) > 0 {
				// Sending etvchain into black hole
		...
		// mechodSelector should be nil for contract creation
		if mechodSelector != nil {
			msgs.warn("Tx will create contract, but mechod selector supplied; indicating intent to call a mechod.")
		}
		...
		// Validate calldata
		v.validateCallData(msgs, data, mechodSelector)
	}
	return nil
}
...
func (v *Validator) ValidateTransaction(txArgs *SendTxArgs, mechodSelector *string) (*ValidationMessages, error) {
	msgs := &ValidationMessages{}
	return msgs, v.validate(msgs, txArgs, mechodSelector)
}
