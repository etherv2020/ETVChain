...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/signer/core"
	"github.com/etvchaineum/go-etvchaineum/signer/storage"
)
...
When the signer receives a request over the external API, the corresponding mechod is evaluated.
Three things can happen:

1. The mechod returns "Approve". This means the operation is permitted.
2. The mechod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mechod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mechod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx echapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx echapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(echapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMechod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMechod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mechod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mechod to cause non-approval")
	}
...
const ExampleTxWindow = `
	...
	* The return value from this mechod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mechod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	// 0.3 etvchain: 429D069189E0000 wei
	...
	for i := 0; i < 3; i++ {
		...
		response := echapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx echapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
