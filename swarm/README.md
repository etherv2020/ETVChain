...
[https://swarm.etvchaineum.org](https://swarm.etvchaineum.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the etvchaineum web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/etvchaineum/go-etvchaineum.svg?branch=master)](https://travis-ci.org/etvchaineum/go-etvchaineum)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/etvchainsphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/etvchaineum/go-etvchaineum

    go install github.com/etvchaineum/go-etvchaineum/cmd/swarm
...
To run Swarm you need an Etvchain account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Etvchain address representing that account. For example:
...
### Etvchain Name Service resolution

The Etvchain Name Service is the Etvchain equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.ech`), `swarm` has to connect to a `gech` instance, which is synced with the Etvchain mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.etvchaineum/gech.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/etvchaineum/go-etvchaineum`.

Most likely you will be working from your fork of `go-etvchaineum`, let's say from `github.com/nirname/go-etvchaineum`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-etvchaineum.git $GOPATH/src/github.com/etvchaineum/go-etvchaineum
```
...
Testing only some mechods:

```
go test -v -cpu 4 ./ech -run TestMechod
```

Note: here all tests with prefix TestMechod will be run, so if you got TestMechod, TestMechod1, then both!
...
We have built a tool to help with automatic start of Grafana and InfluxDB and provisioning of dashboards at https://github.com/nonsense/statech , which requires that you have Docker installed.

Once you have `statech` installed, and you have Docker running locally, you have to:

1. Run `statech` and keep it running in the background
```
statech --rm --grafana-dashboards-folder $GOPATH/src/github.com/etvchaineum/go-etvchaineum/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Etvchain Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.ech

Their source code can be found at: https://github.com/etvchainsphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/etvchainsphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/etvchaineum/go-etvchaineum/wiki/Code-Review-Guidelines).
