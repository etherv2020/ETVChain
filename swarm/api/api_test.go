...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.ech"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.ech"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.ech"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	echAddr := "swarm.ech"
	echHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	echResolve := newTestResolveValidator(echHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(echResolve, "")),
			addr:   echAddr,
			result: echHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(echResolve, ""),
				MultiResolverOptionWithResolver(echResolve, ""),
			),
			addr:   echAddr,
			result: echHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(echResolve, "ech"),
			),
			addr:   echAddr,
			result: echHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "ech"),
				MultiResolverOptionWithResolver(doesntResolve, "ech"),
				MultiResolverOptionWithResolver(echResolve, "ech"),
			),
			addr:   echAddr,
		...
		},
		{
			...
			addr:   echAddr,
			result: echHash,
			err:    errors.New(`DNS name not found: "swarm.ech"`),
		},
		{
