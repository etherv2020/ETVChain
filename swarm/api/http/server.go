...
type mechodHandler map[string]http.Handler

func (m mechodHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	v, ok := m[r.Mechod]
	if ok {
		v.ServeHTTP(rw, r)
		return
	}
	rw.WriteHeader(http.StatusMechodNotAllowed)
}
...
func NewServer(api *api.API, corsString string) *Server {
	...
	c := cors.New(cors.Options{
		...
		AllowedMechods: []string{http.MechodPost, http.MechodGet, http.MechodDelete, http.MechodPatch, http.MechodPut},
		MaxAge:         600,
	...
	mux.Handle("/bzz-raw:/", mechodHandler{
		"GET": Adapt(
	...
	mux.Handle("/bzz-immutable:/", mechodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleBzzGet),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-hash:/", mechodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleGet),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-list:/", mechodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleGetList),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-feed:/", mechodHandler{
		"GET": Adapt(
...
// bufferedReadSeeker wraps bufio.Reader to expose Seek mechod
