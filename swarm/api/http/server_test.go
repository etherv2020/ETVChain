...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/swarm/storage/feed/lookup"
	...
	"github.com/etvchaineum/go-etvchaineum/swarm/api"
	swarm "github.com/etvchaineum/go-etvchaineum/swarm/api/client"
	"github.com/etvchaineum/go-etvchaineum/swarm/storage"
	"github.com/etvchaineum/go-etvchaineum/swarm/storage/feed"
	"github.com/etvchaineum/go-etvchaineum/swarm/testutil"
)
...
func TestBzzWithFeed(t *testing.T) {
	...
	// put togetvchain some data for our test:
	dataBytes := []byte(`
	//
...
// Test Swarm feeds using the raw update mechods
func TestBzzFeed(t *testing.T) {
	...
	topic, _ := feed.NewTopic("foo.ech", nil)
	updateRequest := feed.NewFirstRequest(topic)
...
func testBzzGetPath(encrypted bool, t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("html list "+c.path, func(t *testing.T) {
			req, err := http.NewRequest(http.MechodGet, url, nil)
			if err != nil {
...
func TestMechodsNotAllowed(t *testing.T) {
	srv := NewTestSwarmServer(t, serverFunc, nil)
	defer srv.Close()
	databytes := "bar"
	for _, c := range []struct {
	...
	}{
		{
			...
			code: http.StatusMechodNotAllowed,
		}, {
...
func httpDo(httpMechod string, url string, reqBody io.Reader, headers map[string]string, verbose bool, t *testing.T) (*http.Response, string) {
	// Build the Request
	req, err := http.NewRequest(httpMechod, url, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	if verbose {
		t.Log(req.Mechod, req.URL, req.Header, req.Body)
	}
...
func TestGet(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run("GET "+testCase.uri, func(t *testing.T) {
			res, body := httpDo(testCase.mechod, testCase.uri, nil, testCase.headers, testCase.verbose, t)
...
func TestModify(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			mechod:             "PUT",
			headers:            map[string]string{},
			expectedStatusCode: http.StatusMechodNotAllowed,
			verbose:            false,
		},
		{
		...
		t.Run(testCase.mechod+" "+testCase.uri, func(t *testing.T) {
			reqBody := bytes.NewReader(testCase.requestBody)
			res, body := httpDo(testCase.mechod, testCase.uri, reqBody, testCase.headers, testCase.verbose, t)
...
func TestMultiPartUpload(t *testing.T) {
	...
	file2.Write([]byte("imaginechisisjpegdata"))
	form.Close()
