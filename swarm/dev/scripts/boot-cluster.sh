...
#
# The cluster contains a bootnode, a gech node and multiple swarm nodes, with
...
# static gech configuration
GECH_IP="192.168.33.3"
GECH_RPC_PORT="8545"
GECH_RPC_URL="http://${GECH_IP}:${GECH_RPC_PORT}"
...
# network namespaces togetvchain
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the gech and
# swarm nodes
start_bootnode() {
...
# start_gech_node starts a gech node with --datadir pointing at <base-dir>/gech
# and a single, unlocked account with password "gech"
start_gech_node() {
  local dir="${base_dir}/gech"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gech account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GECH_IP}"
    --rpcport   "${GECH_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gech" "${GECH_IP}" "$(which gech)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vech pair
  local vech0="vech${name}0"
  local vech1="vech${name}1"
  ip link add name "${vech0}" type vech peer name "${vech1}"

  # add one end to the bridge
  ip link set dev "${vech0}" master "${BRIDGE_NAME}"
  ip link set dev "${vech0}" up

  # add the other end to the namespace, rename it ech0 and give it the ip
  ip link set dev "${vech1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vech1}" name "ech0"
  ip netns exec "${name}" ip link set dev "ech0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "ech0"
}
...
create_account() {
  ...
  gech --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
