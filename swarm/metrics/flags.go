...
import (
	...
	gechmetrics "github.com/etvchaineum/go-etvchaineum/metrics"
...
func Setup(ctx *cli.Context) {
	if gechmetrics.Enabled {
		...
		go gechmetrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gechmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(gechmetrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
