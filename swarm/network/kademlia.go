...
func (k *Kademlia) SuggestPeer() (suggestedPeer *BzzAddr, saturationDepth int, changed bool) {
	...
	var pastDepth bool   // whetvchain po of iteration >= depth
...
// i.e., neighbourhood radius is the deepest PO such that all bins not shallower altogetvchain
// contain at least neighbourhoodSize connected peers
// if there is altogetvchain less than neighbourhoodSize peers connected, it returns 0
...
// used for testing only
type Health struct {
	KnowNN           bool     // whetvchain node knows all its neighbours
	CountKnowNN      int      // amount of neighbors known
	MissingKnowNN    [][]byte // which neighbours we should have known but we don't
	ConnectNN        bool     // whetvchain node is connected to all its neighbours
