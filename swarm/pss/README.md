...
The code tutorial [p2p programming in go-etvchaineum](https://github.com/nolash/go-etvchaineum-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-etvchaineum` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-etvchaineum` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
...
In `golang` as special mechod is used:
...
returns:
1. whetvchain key was successfully removed (bool)
```
