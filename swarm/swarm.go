...
import (
	...
	"github.com/etvchaineum/go-etvchaineum/contracts/chequebook"
	"github.com/etvchaineum/go-etvchaineum/contracts/ens"
	...
	"github.com/etvchaineum/go-etvchaineum/p2p/protocols"
	...
	httpapi "github.com/etvchaineum/go-etvchaineum/swarm/api/http"
	"github.com/etvchaineum/go-etvchaineum/swarm/fuse"
	"github.com/etvchaineum/go-etvchaineum/swarm/log"
	"github.com/etvchaineum/go-etvchaineum/swarm/network"
	"github.com/etvchaineum/go-etvchaineum/swarm/network/stream"
	"github.com/etvchaineum/go-etvchaineum/swarm/pss"
	"github.com/etvchaineum/go-etvchaineum/swarm/state"
	"github.com/etvchaineum/go-etvchaineum/swarm/storage"
	"github.com/etvchaineum/go-etvchaineum/swarm/storage/feed"
	"github.com/etvchaineum/go-etvchaineum/swarm/storage/mock"
	"github.com/etvchaineum/go-etvchaineum/swarm/swap"
	"github.com/etvchaineum/go-etvchaineum/swarm/tracing"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = echclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	echClient := echclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, echClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := echclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
