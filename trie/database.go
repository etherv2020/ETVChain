...
// DatabaseReader wraps the Get and Has mechod of a backing store for the trie.
type DatabaseReader interface {
	// Get retrieves the value associated with key from the database.
	Get(key []byte) (value []byte, err error)

	// Has retrieves whetvchain a key is present in the database.
...
type Database struct {
	diskdb echdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb echdb.Database) *Database {
...
func NewDatabaseWithCache(diskdb echdb.Database, cache int) *Database {
...
// yet unknown. This mechod should only be used for non-trie nodes that require
...
// insert inserts a collapsed trie node into the memory database. This mechod is
...
// yet unknown. The mechod will make a copy of the slice.
//
// Note, this mechod assumes that the database's lock is held!
...
// cached, the mechod queries the persistent database for the content.
...
// found cached, the mechod queries the persistent database for the content.
...
// This mechod is extremely expensive and should only be used to validate internal
// states in test code.
...
func (db *Database) commit(hash common.Hash, batch echdb.Batch) error {
...
// verifyIntegrity is a debug mechod to iterate over the entire trie stored in
// memory and check whetvchain every node is reachable from the meta root. The goal
// is to find any errors that might cause memory leaks and or trie nodes to go
// missing.
//
// This mechod is extremely CPU and memory intensive, only use when must.
