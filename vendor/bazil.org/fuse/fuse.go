...
// writing using the various Respond mechods.  This approach is closest to
...
// the service mechods in the interfaces FS* (file system), Node* (file
// or directory), and Handle* (opened file or directory).
// There are a daunting number of such mechods that can be written,
// but few are required.
// The specific mechods are described in the documentation for those interfaces.
//
// The hellofs subdirectory contains a simple illustration of the fs.Serve approach.
//
// Service Mechods
//
// The required and optional mechods for the FS, Node, and Handle interfaces
...
//
// Multiple goroutines may call service mechods simultaneously; the
// mechods being called are responsible for appropriate
// synchronization.
//
...
// Authentication
//
// All requests types embed a Header, meaning that the mechod can
...
// An AccessRequest asks whetvchain the file can be accessed
