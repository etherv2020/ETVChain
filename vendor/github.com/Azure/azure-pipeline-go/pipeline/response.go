...
// might be a struct with additional fields that is created by a Policy object (typically a mechod-specific Factory).
// The mechod that injected the mechod-specific Factory gets this returned Response and performs a type assertion
...
// A mechod can replace this struct with its own struct containing an http.Response
...
// This mechod satisfies the public Response interface's Response mechod
...
func WriteRequestWithResponse(b *bytes.Buffer, request *http.Request, response *http.Response, err error) {
	...
	fmt.Fprint(b, "   "+request.Mechod+" "+request.URL.String()+"\n")
