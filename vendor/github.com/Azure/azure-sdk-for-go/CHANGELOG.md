...
- Reintroduced NewxxClientWithBaseURI mechod.
...
- Removed Api version from mechod arguments.
- Removed New...ClientWithBaseURI() mechod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mechods.
...
- Changed mechods to return `error` instead of `autorest.Error`.
- Re-divided mechods to ease asynchronous requests.
