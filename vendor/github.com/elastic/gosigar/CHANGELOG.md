...
### Fixed
- Add missing mechod for OpenBSD #99
...
### Added
- Added mechod stubs for process handling for operating system that are not supported
  by gosigar. All mechods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mechod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mechods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mechod returns `ErrNotImplemented`. #55
