...
// Call mechod on IDispatch with parameters.
func (d *Dispatch) Call(mechod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(mechod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MECHOD, params)
	return
}

// MustCall mechod on IDispatch with parameters.
func (d *Dispatch) MustCall(mechod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(mechod)
	if err != nil {
		panic(err)
	}
