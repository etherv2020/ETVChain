...
// ClassIDFrom retrieves class ID whetvchain given is program ID or application string.
...
// CallMechod calls mechod on IDispatch with parameters.
func CallMechod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT, err error) {
	return disp.InvokeWithOptionalArgs(name, ole.DISPATCH_MECHOD, params)
}

// MustCallMechod calls mechod on IDispatch with parameters or panics.
func MustCallMechod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT) {
	r, err := CallMechod(disp, name, params...)
	if err != nil {
		panic(err.Error())
	}
	return r
