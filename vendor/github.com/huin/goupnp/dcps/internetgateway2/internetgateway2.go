...
const (
	...
	URN_WANEtvchainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANEtvchainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Etvchainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Etvchainnet
//
...
// WANEtvchainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANEtvchainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANEtvchainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANEtvchainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANEtvchainnetLinkConfig1Clients() (clients []*WANEtvchainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANEtvchainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANEtvchainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANEtvchainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANEtvchainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANEtvchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANEtvchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANEtvchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANEtvchainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANEtvchainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANEtvchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANEtvchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANEtvchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANEtvchainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANEtvchainnetLinkConfig1 {
	clients := make([]*WANEtvchainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANEtvchainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewEtvchainnetLinkStatus: allowed values: Up, Down
func (client *WANEtvchainnetLinkConfig1) GetEtvchainnetLinkStatus() (NewEtvchainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewEtvchainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANEtvchainnetLinkConfig_1, "GetEtvchainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewEtvchainnetLinkStatus, err = soap.UnmarshalString(response.NewEtvchainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
