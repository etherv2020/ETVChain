...
// goroutines which construct valid notify events. This mechod uses
...
// all file descriptors opened by lazyinit mechod and returns afterwards.
...
// It is run in a separate goroutine in order to not block loop mechod when
...
// inotify map. This mechod may also split one raw event into two different ones
...
// decode uses internally stored mask to distinguish whetvchain system-independent
// or system-dependent event is requested. The first one is created by modifying
// `e` argument. decode mechod sets e.event value to 0 when an event should be
...
// This mechod is allowed to return EINVAL error when concurrently requested to
