...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	...
	mechodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHintString {
		mechodSequence = []string{"toString", "valueOf"}
	}
	for _, mechodName := range mechodSequence {
		mechod := self.get(mechodName)
		// FIXME This is redundant...
		if mechod.isCallable() {
			result := mechod._object().call(toValue_object(self), nil, false, nativeFrame)
