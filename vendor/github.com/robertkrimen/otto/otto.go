...
Go translates JavaScript-style regular expressions into someching that is "regexp" compatible via `parser.TransformRegExp`.
...
It would not be difficult to provide someching like these via Go, but you probably want to wrap otto in an event loop in that case.
...
//
// This mechod works by walking the original runtime and cloning each object, scope, stash,
