...
func (runtime *_runtime) newGoStructObject(value reflect.Value) *_object {
	...
	self.class = "Object" // TODO Should this be someching else?
...
func (self _goStructObject) getValue(name string) reflect.Value {
	if validGoStructName(name) {
		...
		if mechod := self.value.MechodByName(name); (mechod != reflect.Value{}) {
			return mechod
		}
	}
...
func (self _goStructObject) mechod(name string) (reflect.Mechod, bool) {
	return reflect.Indirect(self.value).Type().MechodByName(name)
}
...
func goStructEnumerate(self *_object, all bool, each func(string) bool) {
	...
	// Enumerate mechods
	for index := 0; index < object.value.NumMechod(); index++ {
		name := object.value.Type().Mechod(index).Name
		if validGoStructName(name) {
			if !each(name) {
				return
			}
