...
type DB struct {
	...
	inWritePaused          int32 // The indicator whetvchain write operation is paused by compaction
...
// detected in the DB. Use errors.IsCorrupted to test whetvchain an error is
// due to corruption. Corrupted DB can be recovered with Recover function.
//
// The returned DB instance is safe for concurrent use.
// The DB must be closed after use, by calling Close mechod.
...
// the DB.
//
// The iterator must be released after use, by calling Release mechod.
//
...
//
// The snapshot must be released after use, by calling Release mechod.
...
// It is valid to call Close multiple times. Other mechods should not be
