...
func (p *BaggageRestrictionManagerClient) recvGetBaggageRestrictions() (value []*BaggageRestriction, err error) {
	...
	mechod, mTypeId, seqId, err := iprot.ReadMessageBegin()
	if err != nil {
		return
	}
	if mechod != "getBaggageRestrictions" {
		err = thrift.NewTApplicationException(thrift.WRONG_MECHOD_NAME, "getBaggageRestrictions failed: wrong mechod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *BaggageRestrictionManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x3 := thrift.NewTApplicationException(thrift.UNKNOWN_MECHOD, "Unknown function "+name)
