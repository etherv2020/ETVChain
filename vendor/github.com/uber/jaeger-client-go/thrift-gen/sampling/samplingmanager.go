...
func (p *SamplingManagerClient) recvGetSamplingStrategy() (value *SamplingStrategyResponse, err error) {
	...
	if mechod != "getSamplingStrategy" {
		err = thrift.NewTApplicationException(thrift.WRONG_MECHOD_NAME, "getSamplingStrategy failed: wrong mechod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *SamplingManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x4 := thrift.NewTApplicationException(thrift.UNKNOWN_MECHOD, "Unknown function "+name)
